...
The RPM spec file provided here is for Cryptomiles-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://cryptomiles.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Cryptomiles git repository](https://github.com/CryptomilesProject/Cryptomilescoin) but are not part of
...
Currently only `Source10` is used, for the example `cryptomiles.conf` file.
...
in the cryptomiles git repository. At present only an SVG version of the Cryptomiles
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the cryptomiles
...
needed when 0.12.1 is released, a proper fix is already in the Cryptomiles git
master branch.
...
idea to build Cryptomiles Core against a static version of that library compiled
...
The `cryptomiles.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `cryptomiles-server` package. At this point, most RPM based
...
The `cryptomiles-server` package should have SELinux support. How to properly do
...
Cryptomiles-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/cryptomiles/). He has been
packaging Cryptomiles for Fedora at least since 2012.
...
4. I renamed the `cryptomiles` package that contains the Qt GUI to `cryptomiles-core` as
that appears to be how the general population refers to it, in contrast to
`cryptomiles-xt` or `cryptomiles-classic`. I wanted to make sure the general population
