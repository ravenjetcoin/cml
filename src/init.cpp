 ...
 * between cryptomilesd, and cryptomiles-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("cryptomiles-shutoff");
    ...
    FlushWallets();
#endif
    GenerateCryptomiless(false, 0, Params());
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), CRYPTOMILES_CONF_FILENAME));
    if (mode == HMM_CRYPTOMILESD)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), CRYPTOMILES_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/CryptomilesProject/Cryptomilescoin>";
    const std::string URL_WEBSITE = "<https://cryptomilescoin.org>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("cryptomiles-loadblk");
...
/** Sanity checks
 *  Ensure that Cryptomiles is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Cryptomiles version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Cryptomiles process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", CRYPTOMILES_CONF_FILENAME)).string());
    ...
    GenerateCryptomiless(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
