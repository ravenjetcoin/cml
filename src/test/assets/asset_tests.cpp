...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of CRYPTOMILESCOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("CML", type));
        BOOST_CHECK(!IsAssetNameValid("CRYPTOMILES", type));
        BOOST_CHECK(!IsAssetNameValid("CRYPTOMILESCOIN", type));

        //- Versions of CRYPTOMILESCOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("CRYPTOMILES.COIN", type));
        BOOST_CHECK(IsAssetNameValid("CRYPTOMILES_COIN", type));
        BOOST_CHECK(IsAssetNameValid("CMLSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERCML", type));
        BOOST_CHECK(IsAssetNameValid("CRYPTOMILESSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDECRYPTOMILES", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_CRYPTOMILESS", type));
        BOOST_CHECK(IsAssetNameValid("SECMLOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_asset_coin_test)
    {
        ...
        CAssetTransfer asset("CRYPTOMILES", 1000);
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("CRYPTOMILES", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(Params().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
