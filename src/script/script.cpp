...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** CML START */
    case OP_CML_ASSET              : return "OP_CML_ASSET";
    /** CML END */
...
{
    if (this->size() > 30) {
        if ((*this)[25] == OP_CML_ASSET) { // OP_CML_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == CML_R) { // Check to see if CML starts at 27 ( this->size() < 105)
                if ((*this)[28] == CML_V)
                    if ((*this)[29] == CML_N)
                        index = 30;
            } else {
                if ((*this)[28] == CML_R) // Check to see if CML starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == CML_V)
                        if ((*this)[30] == CML_N)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == CML_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == CML_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == CML_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == CML_R) {
