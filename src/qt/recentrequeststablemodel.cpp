    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return CryptomilesUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, CryptomilesUnits::separatorNever);
            else
                return CryptomilesUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != nullptr) ? tr("Requested") + " ("+CryptomilesUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
