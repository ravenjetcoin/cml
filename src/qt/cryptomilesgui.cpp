...
#include "config/cryptomiles-config.h"
#endif

#include "cryptomilesgui.h"
...
const std::string CryptomilesGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString CryptomilesGUI::DEFAULT_WALLET = "~Default";

CryptomilesGUI::CryptomilesGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void CryptomilesGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Cryptomiles address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and cryptomiles: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to CML addresses"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Cryptomiles addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Cryptomiles addresses"));
    ...
    openAction->setStatusTip(tr("Open a cryptomiles: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Cryptomiles command-line options").arg(tr(PACKAGE_NAME)));
...
void CryptomilesGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void CryptomilesGUI::createToolBars()
{
    if(walletFrame)
    {
        /** CML START */
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/cryptomilescointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* cryptomilesLabelLayout = new QVBoxLayout(toolbarWidget);
        cryptomilesLabelLayout->addWidget(label);
        cryptomilesLabelLayout->addWidget(toolbar);
        cryptomilesLabelLayout->setDirection(QBoxLayout::TopToBottom);
        cryptomilesLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Cryptomilescoin Market Price"));
...
void CryptomilesGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool CryptomilesGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool CryptomilesGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void CryptomilesGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void CryptomilesGUI::setWalletActionsEnabled(bool enabled)
{
...
void CryptomilesGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void CryptomilesGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void CryptomilesGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void CryptomilesGUI::optionsClicked()
{
...
void CryptomilesGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void CryptomilesGUI::showDebugWindow()
{
...
void CryptomilesGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void CryptomilesGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void CryptomilesGUI::openClicked()
{
...
void CryptomilesGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void CryptomilesGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void CryptomilesGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void CryptomilesGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void CryptomilesGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void CryptomilesGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** CML START */
void CryptomilesGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void CryptomilesGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void CryptomilesGUI::gotoManageAssetsPage()
{
...
void CryptomilesGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Cryptomiles network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void CryptomilesGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void CryptomilesGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void CryptomilesGUI::updateHeadersSyncProgressLabel()
{
...
void CryptomilesGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void CryptomilesGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Cryptomiles"); // default title
...
void CryptomilesGUI::changeEvent(QEvent *e)
{
...
void CryptomilesGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void CryptomilesGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void CryptomilesGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "CML")
        msg += tr("Amount: %1\n").arg(CryptomilesUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(CryptomilesUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void CryptomilesGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void CryptomilesGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool CryptomilesGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool CryptomilesGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void CryptomilesGUI::setHDStatus(int hdEnabled)
{
...
void CryptomilesGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void CryptomilesGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void CryptomilesGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void CryptomilesGUI::detectShutdown()
{
...
void CryptomilesGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void CryptomilesGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void CryptomilesGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(CryptomilesGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void CryptomilesGUI::subscribeToCoreSignals()
{
...
void CryptomilesGUI::unsubscribeFromCoreSignals()
{
...
void CryptomilesGUI::toggleNetworkActive()
{
    if (clientModel) {
    ...
    menu(0)
{
    ...
    QList<CryptomilesUnits::Unit> units = CryptomilesUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const CryptomilesUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(CryptomilesUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    for (CryptomilesUnits::Unit u : CryptomilesUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(CryptomilesUnits::name(u)), this);
...
{
    setText(CryptomilesUnits::name(newUnits));
}
...
void CryptomilesGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=CMLBTC"));
